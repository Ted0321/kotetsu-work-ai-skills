...
## メイン投稿（通常投稿・139.5字換算＝確定版）

```text
Codexで話題の、古いキャッシュを毎週自動で掃除してくれる機能。Claude Codeには無いので、スクリプトをスケジュール登録して再現しました。

あとはClaude Codeが毎週勝手にお掃除してくれる。古いログだけ消えるので、肥大化が止まりストレージも圧迫しません。

解説と導入プロンプトはリプ欄に🧵
```

控えめ版（実行主体をぼかす・139字換算）:
