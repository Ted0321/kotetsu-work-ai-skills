...
添付画像:
- 1枚目（Before）= 自分のターミナルの実スクショ、または実測値で作成済みの `assets/before-real.png`
- 2枚目（After）= 本体プロンプト実行後に出る「おそうじレポート」のスクショ（実行後に確定）

```text
Claude Code、使うほどローカルにデータが溜まり続けてるの、知ってました？
実測したら662MB。ほぼ全部が「過去の会話ログ」でした。
...
- Before画像: 「診断コマンドの出力。Claude Codeのローカルデータが合計662.5MB、うち662MBが過去の会話ログ」
- After画像: 「おそうじレポート。回収量と、次回は毎週月曜9時に自動実行される旨の表示」（回収量はレシート確定後に記入）
