...
## メイン投稿（通常投稿・140字版 = 139字換算）

```text
Codexで話題の「古いキャッシュの自動クリア」、Claude Codeでも再現しました。

同機能が無いため、プロンプト1回で掃除スクリプトを作りOSスケジューラに週次登録する方式に。

以後は放置OK。ログの肥大化が止まり、毎週の実行はAI不使用＝コスト0・確認なし。

解説と導入プロンプトはリプ欄に🧵
```

予備（138字換算・「容量回収」を入れる版）:

```text
Codexで話題の「古いキャッシュ自動クリア」をClaude Codeで再現しました。

標準に無い機能なので、プロンプト1回で掃除スクリプトを週次スケジューラに登録する方式に。

会話ログ等の肥大化が止まり容量も回収。毎週の実行はAI不使用でコスト0、確認も不要。
